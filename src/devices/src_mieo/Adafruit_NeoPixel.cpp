...
//emotion fuctions
void Adafruit_NeoPixel::showEmotion(const uint8_t* face, uint32_t color) {
  clear();
  for (uint8_t row = 0; row < 5; row++) {
    for (uint8_t col = 0; col < 7; col++) {
      if (face[row] & (1 << (6 - col))) {
        uint16_t index = row * 7 + col;
...
void Adafruit_NeoPixel::displayChar(char c, uint32_t color) {
  const uint8_t* bitmap = nullptr;

  if (c >= 'A' && c <= 'Z') bitmap = font_5x7[c - 'A'];
  else if (c >= '0' && c <= '9') bitmap = font_5x7[c - '0' + 26];
  else if (c >= 'a' && c <= 'z') bitmap = font_lower_special[c - 'a'];
  else {
    const char specials[] = " !?.:,;'_-+=/\\$#@";
    for (uint8_t i = 0; i < sizeof(specials) - 1; i++) {
      if (c == specials[i]) {
        bitmap = font_lower_special[26 + i];
        break;
      }
    }
  }
...
void Adafruit_NeoPixel::displayText(const char* text, uint32_t color, uint16_t speed) {
  uint8_t scrollBuffer[256] = {0};
  uint8_t pos = 0;
  uint8_t length = strlen(text);

  if (length == 1) {
    displayChar(text[0], color);
    return;
  }
  ...
  for (uint8_t i = 0; i < length; i++) {
    ...
    if (c >= 'A' && c <= 'Z') bitmap = font_5x7[c - 'A'];
      ...
      const char specials[] = " !?.:,;'_-+=/\\$#@";
      for (uint8_t j = 0; j < sizeof(specials) - 1; j++) {
        if (c == specials[j]) {
          bitmap = font_lower_special[26 + j];
          break;
        }
      }
    }
    ...
    for (uint8_t col = 0; col < 7; col++) {
      uint8_t column = 0;
      for (uint8_t row = 0; row < 5; row++) {
        if (bitmap && (bitmap[row] & (1 << (6 - col)))) {
          column |= (1 << (6 - row));
        }
      }
      scrollBuffer[pos++] = column;
    }
  }

  for (uint8_t i = 0; i < WIDTH; i++) scrollBuffer[pos++] = 0;

  for (int offset = 0; offset <= pos - WIDTH; offset++) {
    clear();
    for (uint8_t row = 0; row < 5; row++) {
      for (uint8_t col = 0; col < WIDTH; col++) {
        if (scrollBuffer[offset + col] & (1 << (6 - row))) {
          int idx = getLedIndex(row, col);
          setPixelColor(idx, color);
        }
      }
